...
-- Admin policy for viewing all orders (optional - for admin dashboard)
CREATE POLICY "Admins can view all orders" ON public.orders
    FOR SELECT USING (
        EXISTS (
            SELECT 1 FROM public.profiles 
            WHERE id = auth.uid() 
            AND (slug = 'admin' OR email = 'admin@tapverse.com')
        )
    );

-- Create order_items table for future extensibility (if users can order multiple different designs)
